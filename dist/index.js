...
function resolveReccursiveTree(tree) {
  return __async(this, null, function* () {
    var _a, _b;
    try {
      if (!tree.rest) {
        yield (_a = tree.current) == null ? void 0 : _a.call(tree);
        return;
      }
      yield (_b = tree.current) == null ? void 0 : _b.call(tree);
      tree.rest.forEach((branch) => __async(this, null, function* () {
        return yield resolveReccursiveTree(branch);
      }));
    } catch (error) {
      console.log(error);
    }
...
function makeAction(path) {
  return __async(this, null, function* () {
    ...
    console.log(path);
    const folderPromise = writeFolderPromise_default(component_default.root, path);
...
// index.ts
program.command("make <path>").description("Make a folder using template").option("-t, --template").action((path) => make_default(path));
program.parse();
